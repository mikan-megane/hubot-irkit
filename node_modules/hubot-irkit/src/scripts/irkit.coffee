...
module.exports = (robot) ->
  ...
  getDevice = (msg, name, silent = no)->
    ...
    if !(device = devices[name]) && !silent && msg
      if msg == bot
        robot.messageRoom 'cron','Device: #{name} is not registered.'
      else
  ...
  getMessage = (msg, deviceName, messageName, silent = no)->
    if device = getDevice msg, deviceName
      ...
      if !(message = device.messages[messageName]) && !silent && msg
        if msg == bot
          robot.messageRoom 'cron','エラー: #{messageName} は登録されていないかスペルが違います。'
        else
  ...
  sendIRMessage = (msg, deviceName, messageName, callback)->
    ...
    if msg == bot
      robot.messageRoom 'cron', "送信しています・・・"
    else
      msg?.send "送信しています・・・"
  ...
  robot.hear /\s*([^\s]+?)を?(?:つけて|点けて)\s*$/i, (msg) ->
    ...
    sendIRMessage msg, 'home', messageName, (err, res, body) ->
      if res?.statusCode == 200
        msg.send "完了しました。"
      else
        msg.send "送信できませんでした。"
  ...
  new cronJob( '0 0,30 20,21,22,23,0 * * *', () ->
    msg = bot
    sendIRMessage msg, 'home', 'down,電気', (err, res, body) ->
      if res?.statusCode == 200
  ...
  new cronJob( '15 0 21,22,23,0 * * *', () ->
    msg = bot
    sendIRMessage msg, 'home', 'down,にこの電気', (err, res, body) ->
      if res?.statusCode == 200
        robot.messageRoom 'cron', "にこの電気を暗くしました"
      else
        robot.messageRoom 'cron', "にこの電気を暗くできませんでした。"
  ).start()
  new cronJob( '15 * 21,22,23,0 * * *', () ->
    msg = bot
    sendIRMessage msg, 'home', 'test', (err, res, body) ->
      if res?.statusCode == 200
        robot.messageRoom 'cron', "test true"
      else
        robot.messageRoom 'cron', "test false"
  ).start()