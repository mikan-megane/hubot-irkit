...
module.exports = (robot) ->
  ...
  robot.respond /\s*(?:登録|とうろく)\s*((?:on|off|up|down|[0-9]{1,2}),.+)\s*$/i, (msg) ->
  ...
  robot.respond /\s*([^\s]+?)を?忘れて\s*$/i, (msg) ->
    messageName = msg.match[1]
    deviceName = 'home'
    return unless device = getDevice msg, deviceName
  ...
  robot.respond /\s*([^\s]+?)を?(?:つけて|点けて)\s*$/i, (msg) ->
    ...
    sendIRMessage msg, deviceName, messageName, (err, res, body) ->
      if res?.statusCode == 200
        ...
        msg.send "送信できませんでした。"
  robot.respond /\s*([^\s]+?)を?(?:消して|けして)\s*$/i, (msg) ->
    ...
    sendIRMessage msg, deviceName, messageName, (err, res, body) ->
      if res?.statusCode == 200
        ...
        msg.send "送信できませんでした。"
  robot.respond /\s*([^\s]+?)を?(?:上げて|明るくして)\s*$/i, (msg) ->
    ...
    sendIRMessage msg, deviceName, messageName, (err, res, body) ->
      if res?.statusCode == 200
        ...
        msg.send "送信できませんでした。"
  robot.respond /\s*([^\s]+?)を?(?:下げて|暗くして)\s*$/i, (msg) ->
    ...
    sendIRMessage msg, deviceName, messageName, (err, res, body) ->
      if res?.statusCode == 200
        ...
        msg.send "送信できませんでした。"
  robot.respond /\s*([^\s]+?)を?([0-9]{0,2}?)(?:にして)\s*$/i, (msg) ->
    if msg.length == 0 
      messageName = 'on,' + msg.match[1]
    else
      messageName = msg.match[2] + ',' + msg.match[1]
    deviceName = 'home'
    sendIRMessage msg, deviceName, messageName, (err, res, body) ->
      if res?.statusCode == 200
        msg.send "完了しました。"
